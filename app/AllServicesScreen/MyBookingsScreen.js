...
const MyBookingsScreen = ({ navigation }) => {
  ...
  const fetchBookings = async (pageNum = 1, isRefresh = false) => {
    try {
      ...
      const response = await otherServices.getMyAmenityBookings(pageNum);

      if (response?.status === "success") {
        const incoming = response.data || [];
        setBookings((prev) => pageNum === 1 ? incoming : [...prev, ...incoming]);
        const perPage = response.metadata?.perPage || 20;
        setHasMore(incoming.length >= perPage && incoming.length > 0);
      } else {
